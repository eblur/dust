...
def test_units():
    assert Grain().ndens(0.0, TEST_RHO) == 0.0
    assert np.sum(Powerlaw().ndens(0.0, TEST_RHO)) == 0.0
...
def test_ndens_units(gmodels):
    test = gmodels
    assert np.all(test.ndens(2.0*TEST_MD, TEST_RHO)/test.ndens(TEST_MD, TEST_RHO) == 2.0)
...
@pytest.mark.parametrize('comps', ['Silicate','Graphite'])
def test_WD_md(comps):
    test = WD01(comp=comps)
    mg   = (4./3.) * np.pi * test.comp.rho * np.power(test.a * c.micron2cm, 3)
    md_test = trapz(test.nd_nom * mg, test.a)
    assert np.abs(md_test/test.md_nom - 1.0) < 0.01
